...
uint256 hashGenesisBlock("0x0000073b92fb82809c0e883cb62133c9e0086cfce17d27e64397e76564c6219a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chipcoin: starting difficulty is 1 / 2^12
...
{
    // Chipcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 0;
    if (nHeight <= 5) {    // For Each 5 blocks will have 14K coins
       nSubsidy = 14000 * COIN;
    }
    else {
       nSubsidy = 512 * COIN;
    }
    // Subsidy is cut in half every 10,000 blocks,
    nSubsidy >>= (nHeight / 10000); // Chipcoin: 10k blocks
    //
    if (nSubsidy < COIN) nSubsidy = COIN;  // Minimum Number of Coin = 1
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60;                                   // Chipcoin: Difficulty adjusted every 10 mintues
static const int64 nTargetSpacing =   1 * 60;                                   // Chipcoin: Every Minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chipcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Chipcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043124710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1437441754;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1239457;
        ...
        assert(block.hashMerkleRoot == uint256("0xc3f99a0f9c2912066ffc267c93abd9d817783cb5279d66e67c951c191f8d2404"));
...
unsigned char pchMessageStart[4] = { 0xd1, 0xd2, 0xd1, 0xdc }; // Chipcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChipcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChipcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChipcoinMiner(CWallet *pwallet)
{
    printf("ChipcoinMiner started\n");
    ...
    try { for (;;) {
        ...
        printf("Running ChipcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChipcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChipcoinMiner, pwallet));
}
