    ...
    currentImmatureBalance(-1)
{
    ...
    backGround->setStyleSheet("background: #191923"); //background-color: QLinearGradient( x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #191923, stop: 1 #87878D)
    ...
    chipCoinLogo = new ClickableLabel(this);
    chipCoinLogo->setPixmap( QPixmap(":/icons/gui_header_logo"));
    chipCoinLogo->setContentsMargins(0,0,0,0);
    chipCoinLogo->move(15,35-10);
    chipCoinLogo->adjustSize();
    ...
    walletOverviewBox->setStyleSheet(".QWidget {background-color: rgba(0,0,0, 0);}");
    ...
    walletTitle->setText("Bank <font color=#000000>(out of sync)</font>");
    ...
    recentOverviewBox->setStyleSheet(".QWidget {background-color: rgba(0,0,0, 0);}");

    QLabel *recentTitle = new QLabel(recentOverview);
    recentTitle->setStyleSheet("QLabel { color: #FFFFFF; }");
    recentTitle->setFont(font);
    recentTitle->setText("Recent Transactions");
    recentTitle->setFixedWidth(160);
    ...
    networkOverviewBox->setStyleSheet(".QWidget {background-color: rgba(0,0,0, 0);}");
    ...
    networkTitle->setText("Network Statistics <font color=#000000>(out of sync)</font>");
    networkTitle->setFixedWidth(190);
...
void GuiHeader::showOutOfSyncWarning(bool fShow)
{
    if(fShow){
        ...
        walletTitle->setText("Bank");
        networkTitle->setText("Network Statistics");
    }
}
...
void GuiHeader::updateNetworkOverview()
{
   ...
   // int totalMoney = (double)GetTotalSupply()/(double)COIN;
    //moneySupply->setText(QString::number( totalMoney)+" CHIP");
