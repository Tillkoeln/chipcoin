...
void BlockBrowser::keyPressEvent(QKeyEvent *event)
{
...
void BlockBrowser::showEvent(QShowEvent*)
{
    if (m_NeverShown)
    {
        m_NeverShown = false;
        ...
        {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (chipcoin.conf).");
            QMessageBox::warning(this, "Chipcoin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}

bool BlockBrowser::switchTo(const QString& query)
{
    bool IsOk;
...
void BlockBrowser::goTo(const QString& query)
{
    if (switchTo(query))
    {
...
void BlockBrowser::onSearch()
{
    goTo(ui->searchBox->text());
}

void BlockBrowser::setBlock(CBlockIndex* pBlock)
{
    setContent(BlockToString(pBlock));
}

void BlockBrowser::setContent(const std::string& Content)
{
...
void BlockBrowser::updateNavButtons()
{
